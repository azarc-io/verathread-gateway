...
// WebsocketSubscriptionClient is an actual implementation of the subscriptions client interface.
...
// TODO change to non deprecated interface
//
//nolint:all
...
func (g *GraphQLHTTPRequestHandler) handleWebsocket(connInitReqCtx context.Context, conn net.Conn) {
	...
	go HandleWebsocket(done, errChan, conn, executorPool, logger.ZapLogger(g.log))
	select {
	case err := <-errChan:
		g.log.Error().Err(err).Msgf("http.GraphQLHTTPRequestHandler.handleWebsocket()")
	case <-done:
	}
}
